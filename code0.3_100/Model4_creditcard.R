...
#functions
{
  ...
  streaming_score = function(input, Min = 100, start_index = start_index, T=0.2, Max = 0, until=0, batch = TRUE ,embedding_size_p = 0, remove=TRUE, remove_threshold = 0.2){
...
#Result
{
  ...
  for(i in part){
    output = streaming_score(input, Min=100, start_index= i, Max=100, until = 299, batch=TRUE, remove= TRUE, embedding_size_p=0, T=0.2, eMax= 0)  # onehot
    if(is.null(output) == 0 ){
  ...
  setwd("/home/jonghyeon3/extension_AD/evaluations/total_result/result0.3_100")
  write.csv(output_total, "result_model4_creditcard.csv", row.names= FALSE)
}
