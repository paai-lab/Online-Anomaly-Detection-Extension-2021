...
for(k in 1:10){
  ...
  pre2 = pre[which(!is.na(pre$Case)),]
    ...
    data <- pre2[,c("Case","Activity","order", "Event")] 
    ...
    cc= cbind(c, act= (c$anomaly_type != "normal") ,pred = (h_diag>0.5))
    
    subresult=  cc
    
    
    
    cm4 <- confusionMatrix( as.factor(as.numeric(subresult$pred)), as.factor(as.numeric(subresult$act)),positive = '1')
  ...
  print(sum( subresult$pred==subresult$act)/nrow(subresult))
...
write.csv(result, "offline_ocsvm_trace.csv", row.names = F)
