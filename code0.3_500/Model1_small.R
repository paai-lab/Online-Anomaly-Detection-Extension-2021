...
normal_seq = aggregate(normal$Activity, by=list(normal$Case), FUN= function(x) paste(x, collapse = '>>'))
anomaly_seq = aggregate(anomaly$Activity, by=list(anomaly$Case), FUN= function(x) paste(x, collapse = '>>'))
...
#Result
{
  ...
  setwd("/home/jonghyeon3/extension_AD/evaluations/total_result/result0.3_500")
  write.csv(output_total, "result_model1_small.csv", row.names= FALSE)
}

Min=100; start_index= part[1]; Max=500; until = 299; batch=FALSE; remove= FALSE; embedding_size_p=0, T=0.2
