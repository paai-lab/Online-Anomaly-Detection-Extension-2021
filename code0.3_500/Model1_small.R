...
#Result
{
  ...
  for(i in part){
    output = streaming_score(input, Min=100, start_index= i, Max=500, until = 299, batch=FALSE, remove= FALSE, embedding_size_p=0)  # onehot
    if(is.null(output) == 0 ){
...
Min=100; start_index= part[1]; Max=500; until = 299; batch=FALSE; remove= FALSE; embedding_size_p=0
