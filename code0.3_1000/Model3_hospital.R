...
#Result
{
  ...
  for(i in part){
    output = streaming_score(input, Min=100, start_index= i, Max=1000, until = 299, batch=FALSE, remove= TRUE, embedding_size_p=0, T=0.2, eMax=0)  # onehot
    if(is.null(output) == 0 ){
  ...
  write.csv(output_total, "result_model3_hospital.csv", row.names= FALSE)
}
