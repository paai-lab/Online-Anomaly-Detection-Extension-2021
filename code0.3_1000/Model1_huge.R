...
#functions
{
  ...
  fun_remove_FALSE = function(input, Min, start_index, Max, until,T, eMax, embedding_size_p){
    ...
    for(i in (start_index+1):until){ # last_index
      ...
      t <- try(tryCatch(fun_leverage(x),  error = function(e) e))
      if("simpleError" %in% class(t) ==1){
        pre[i, 'w-leverage'] = 0
        pre[i, 'w-tn'] =  0
        pre[i, 'leverage'] = 0
        leverage_end <- Sys.time()
...
#Result
{
  ...
  setwd("/home/jonghyeon3/extension_AD/evaluations/total_result/result0.3_1000")
  write.csv(output_total, "result_model1_huge.csv", row.names= FALSE)
}
