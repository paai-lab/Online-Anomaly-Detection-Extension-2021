...
#Result
{
  ...
  for(i in part){
    output = streaming_score(input, Min=100, start_index= i, Max=100, until = 299, batch=TRUE, remove= FALSE, embedding_size_p=0)  # onehot
    if(is.null(output) == 0 ){
  ...
  setwd("/home/jonghyeon3/extension_AD/evaluations/bs1/result")
  write.csv(output_total, "result_svm100_hospital.csv", row.names= FALSE)
}
