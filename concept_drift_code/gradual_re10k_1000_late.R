...
{
input = data.frame(read.csv('gradual_re10k.csv', header=T))
input = input[which(is.element(input$Case, unique(input$Case)[1501:2000])),]
...
#Result
{
  output = streaming_score(input, Min=100,  Max=1000, until = 0, batch=TRUE, remove= FALSE, embedding_size_p=0)  # onehot

  
  setwd("~/extension_AD/evaluations/total_result/concept_drift_result")
  write.csv(output, "result_model2_gradual_re10k_1000_late.csv", row.names= FALSE)
}
