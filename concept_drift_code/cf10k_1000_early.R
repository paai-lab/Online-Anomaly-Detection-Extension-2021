...
{
input = data.frame(read.csv('cf10k-0.1-0.csv', header=T))
input = input[which(is.element(input$Case, unique(input$Case)[1:1000])),]
...
#Result
{
  ...
  write.csv(output, "result_model2_cf10k_1000_early.csv", row.names= FALSE)
}
