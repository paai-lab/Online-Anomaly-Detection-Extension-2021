...
{
input = data.frame(read.csv('re10k-0.1-0.csv', header=T))
...
#Result
{
  output = streaming_score(input, Min=100,  Max=300, until = 0, batch=TRUE, remove= FALSE, embedding_size_p=0)  # onehot

  
  setwd("~/extension_AD/evaluations/total_result/concept_drift_result")
  write.csv(output, "result_model2_re10k_300_early.csv", row.names= FALSE)
}
