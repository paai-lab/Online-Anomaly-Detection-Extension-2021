...
library(MASS); library(IsolationForest)
library(caret)
library(fGarch)
...
{
input = data.frame(read.csv('hospitalbilling-0.1-0.csv', header=T))
input$Case[which(is.na(input$Case))] = 'NA'
...
#functions
{
  ...
  fun_svm = function(x){
    if(sum( apply(x, 2, FUN= function(x){length(unique(x))} ) > 1  )>0){
      x2= x[, which(apply(x, 2, FUN= function(x){length(unique(x))} ) > 1 )]
      
      svm.model<-svm(x2,y=NULL,
                     type='one-classification',
                     nu=0.5,
                     scale=TRUE,
                     kernel="radial")
      
      svm.predtrain<-predict(svm.model,x2)
      return(svm.predtrain)
    }else{
      svm.predtrain = rep(0, nrow(x))
      return(svm.predtrain)
    }
  }
...
#Result
{
  ...
  for(i in part){
    output = streaming_score(input, Min=100, start_index= i, Max=1000, until = 299, batch=FALSE, remove= FALSE, embedding_size_p=0, T=0.2, eMax= 0)  # onehot
    if(is.null(output) == 0 ){
  ...
  setwd("/home/jonghyeon3/extension_AD/evaluations/bs1/result_trace")
  write.csv(output_total, "result_itree1000_hospital.csv", row.names= FALSE)
}
